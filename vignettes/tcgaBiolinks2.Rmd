...
for ( i in 1: nrow(matSamples)){
 ...
 colnames(matSamples) <- substr( colnames(matSamples), 1,2)

knitr::kable(matSamples, digits = 2, caption = "Table common samples among platforms from TCGAQuery",row.names = T)
```
...
# You can define a list of samples to query and download proving relative TCGA barcodes.
...
# Query about all platform available with a list of barcode 
query <- TCGAQuery(samples = listSamples)

# Query about platform IlluminaHiSeq_RNASeqV2 available with a list of barcode 
query <- TCGAQuery(tumor = "lusc", samples = listSamples, platform = "IlluminaHiSeq_RNASeqV2", level = "3")

# Download a list of barcode with platform IlluminaHiSeq_RNASeqV2
TCGADownload(query, path = "dataLusc", type = "rsem.genes.results",samples = listSamples)

# Prepare expression matrix with gene id in rows and samples (barcode) in columns and rsem.genes.results as values
...
knitr::kable(dataGE, digits = 2, caption = "Matrix Gene expression (genes in rows and samples in columns)",row.names = F)
```
...
```{r, eval = TRUE, echo = FALSE}
library(TCGAbiolinks)
dataDEGsFiltLevel$FDR <- format(dataDEGsFiltLevel$FDR, scientific = T)
knitr::kable(dataDEGsFiltLevel[1:10,], digits = 2, caption = "Table DEGs after DEA",row.names = F)
```
