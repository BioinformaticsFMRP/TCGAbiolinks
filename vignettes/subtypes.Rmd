...
The functions `PanCan2018_subtypes` and `TCGAquery_subtype` can be used to get the information tables.

While the `PanCan2018_subtypes` function gives access to a curated table retrieved from synapse (probably with the most updated  molecular subtypes)
the `TCGAquery_subtype` function has the complete table also with sample information retrieved from the TCGA marker papers.

# `PanCan2018_subtypes`: Curated molecular subtypes.

Data and description retrieved from synapse (https://www.synapse.org/#!Synapse:syn8402849) 

Synapse has published a single file with all available molecular subtypes that have been described by TCGA (all tumor types and all molecular platforms), which can be accessed using the `PanCan2018_subtypes` function as below:

```{r message=FALSE, warning=FALSE}
subtypes <- PanCan2018_subtypes()
