...
The functions `PanCan33_subtypesLink` and `TCGAquery_subtype` can be used to get the information tables.

While the `PanCan33_subtypesLink` function gives access to a curated table retrieved from synapse (probably with the most updated  molecular subtypes)
the `TCGAquery_subtype` function has the complete table also with sample information retrieved from the TCGA marker papers.

# `PanCan33_subtypesLink`: Curated molecular subtypes.

Data and description retrieved from synapse (https://www.synapse.org/#!Synapse:syn8402849) 

Synapse has published a single file with all available molecular subtypes that have been described by TCGA (all tumor types and all molecular platforms), which can be accessed using the `PanCan33_subtypesLink` function as below:

```{r message=FALSE, warning=FALSE}
subtypes <- PanCan33_subtypesLink()
...
| BLCA         | https://www.ncbi.nlm.nih.gov/pubmed/24476821              | Comprehensive molecular characterization of urothelial bladder carcinoma.
                                               |        Nature 2014          |
