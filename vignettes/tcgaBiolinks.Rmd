...
# `TCGAVersion`: Summary versions of the data in TCGA 
...
object for downstream analysis. For the moment this function is working only
with data level 3.
...
# checking summarized experiment with data.frame

#BRCAMatrix_data <- BRCAMatrix@assays$data$raw_counts
#colnames(BRCAMatrix_data)<-BRCAMatrix@colData$sample
#rownames(BRCAMatrix_data)<-BRCAMatrix@rowRanges@elementMetadata$gene_id
BRCAMatrix_assay <- assay(BRCAMatrix,1)
commonGenes <- intersect(rownames(BRCAMatrix_assay), rownames(dataBRCA))
sum(dataBRCA[commonGenes,] != BRCAMatrix_assay[commonGenes,])
```
...
#  `clinic` `&` `clinicFilt`: Working with clinical data
...
For example the function `clinicFilt` will filter your data, returning the list 
...
bar <- c("TCGA-G9-6378-02A-11R-1789-07", "TCGA-CH-5767-04A-11R-1789-07",  
        ...
        "TCGA-G9-6380-11A-11R-1789-07", "TCGA-G9-6380-01A-11R-1789-07",
        "TCGA-G9-6340-01A-11R-1789-07","TCGA-G9-6340-11A-11R-1789-07")
female_erpos_herpos <- clinicFilt(bar, clinBRCA, HER = "Positive", 
...
function `RnaSeqNormalization`, do a quantile filter of genes with
the `RnaSeqFilt` function. Also, iin order to classify your samples (barcode) 
you can use the `MultiSampleTypes` function, the typeSample "NT" will return 
...
## `DEArnaSEQ` `&` `CreateTabLevel` Differential expression analysis (DEA)
...
`CreateTabLevel` function to create a table with DEGs (differentially expressed genes), log Fold Change (FC), false discovery rate (FDR), the gene expression level for samples in Cond1type, and Cond2type, and Delta value (the difference of gene expression between the two conditions multiplied logFC).
...
## `EAcomplete & EAbarplot`: Enrichment Analysis
...
## `plotPCAforGroups`: Principal Component Analysis plot for differentially expressed genes

In order to understand better our genes, we can perform a PCA to reduce the
number of dimensions of our gene set. The function `plotPCAforGroups` will plot
the PCA for different groups.

The parameters of this function are:
* **dataFilt** The expression matrix after normalization and quantile filter
* **dataDEGsFiltLevel** The CreateTabLevel output
...
tokenStart <- tokenStop
tokenStop <-100*i
tabSurvKM<-SurvivalKMunivariate(clinical_patient_Cancer,
                                dataBRCAcomplete,
                                Genelist = rownames(dataBRCAcomplete)[tokenStart:tokenStop],
                                Survresult = F,ThreshTop=0.67,ThreshDown=0.33)
...
TabCoxNet <- SurvivalCoxNET(clinical_patient,dataBRCAcomplete,
                            Genelist = rownames(tabSurvKMcomplete_onlyTF),
                            scoreConfidence = 700,titlePlot = "SurvivalCoxNET Example")
```
The result is shown below:
```{r, fig.width=6, fig.height=4, echo=FALSE, fig.align="center"}
...
DEGsBlue <- DEArnaSEQ(cbind(GroupGreen3Data, GroupRedData), GroupBlueData, 
    "GroupOther", "GroupBlue")
DEGsGreen3 <- DEArnaSEQ(cbind(GroupBlueData, GroupRedData), GroupGreen3Data, 
    "GroupOther", "GroupGreen3")
DEGsRed <- DEArnaSEQ(cbind(GroupBlueData, GroupGreen3Data), GroupRedData, 
    "GroupOther", "GroupRed")


dataDEGs <- DEArnaSEQ(dataFilt[, samplesNT], dataFilt[, samplesTP], "Normal", 
    "Tumor")

# DEGs filter by abs(logFC) >=1
dataDEGsFilt <- dataDEGs[abs(dataDEGs$logFC) >= 1, ]

dataDEGsFiltLevel <- CreateTabLevel(dataDEGsFilt, "Tumor", "Normal", dataFilt[, 
    samplesTP], dataFilt[, samplesNT])

DEGsBlueLevel <- CreateTabLevel(DEGsBlue, "GroupBlue", "GroupOther", GroupBlueData, 
...
DEGsRedLevel <- CreateTabLevel(DEGsRed, "GroupRed", "GroupOther", GroupRedData, 
...
## `meanMethylationAnalysis`: Mean Methylation Analysis
Using the data and calculating the mean methylation per group, it is possible 
to create a mean methylation boxplot with the function `meanMethylationAnalysis`
as follows:
...
## `DMRAnalysis`: Differentially methylated regions Analysis

We will search for differentially methylated CpG sites using the `DMRAnalysis`
...
```{r, eval = FALSE}
data <- DMRAnalysis(data,groupCol="group")
```
...
The output will be an graph such as the figure below. Also, the `DMRAnalysis` 
...
## `starburstAnalysis`: Analyzing expression and methylation together 
 ...
 reproduce this plot, we will use the `starburstAnalysis` function.
...
`TCGAPrepare` function  and processed by `DEArnaSEQ` function. Expected colData columns: diffmean, p.value.adj and p.value
...
starburstAnalysis(met,exp,"group1","group2")
```
...
## `TCGAquestion`: Searching questions,answers and literature
The `TCGAquestion` function has two type of searches, one that searches for
most downloaded packages in CRAN or BioConductor and one that searches the most related question in biostar. 

### `TCGAquestion` with BioConductor
...
tabPackage <- TCGAquestion(siteToFind ="bioconductor.org",listPackage)


# define a keyword to find in support.bioconductor.org returing a table with suggested packages
tabPackageKey <- TCGAquestion(siteToFind ="support.bioconductor.org" ,KeyInfo = "tcga")
```
...
tabPackage1 <- TCGAquestion(siteToFind ="biostars.org",KeyInfo = "TCGA")

# Find most related question in biostar with package
tabPackage2 <- TCGAquestion(siteToFind ="biostars.org",KeyInfo = "package")
...
library(TCGAbiolinks)
# Find most related question in biostar with TCGA
#tabPackage <- TCGAquestion(siteToFind ="biostars.org",KeyInfo = "package")
