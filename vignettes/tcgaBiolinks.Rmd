...
#load("../LGG_caseStudy/dataNorm.RData")
...
save(data_Hc2, file = "../LGG_caseStudy/data_Hc2.Rdata", compress = "xz")
...
TCGAanalyze_survival(plyr::rbind.fill(dataClin),
                     clusterCol = "groupsHC",
                     main = "Kaplan meier survival plot from consensus cluster",
                     height = 10, 
                     width=10)
...
## to integrate in TCGAvisualize_Clustering
grpsHC <- levels(consensusClusters)
...
plot(data_Hc1, labels = FALSE, main =paste0(cancer, " cluster dendrogram all samples",
     xlab = "Samples with relative group color"), xlab = "Hierarchical cluster cut k=4")

rect.hclust(sHc, k=4, border="red")
tabCluster <- as.matrix(cutree(sHc, k = 4))
...
for( i in 1:length(grpsHC)){
rug(which(tabCluster[sHc$order, "Color"] == ccol[i]), col = ccol[i], lwd = 3)
}
#  to be continued...
