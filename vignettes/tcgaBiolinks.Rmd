...
A full description of the clinical data files provided by TCGA is below:
...
* Clinical (patient information):
 ...
 + clinical_patient_updated: this is TCGAbiolinks only. This options downloads the clinical_patient file and updates the information using the last follow-up information that is available for the patient.
...
The files available for each tumor type is shown below:
```{r, eval = TRUE, echo = FALSE}
knitr::kable(clinical.table, 
             caption = "List of clinical files: 1 means the file exists, 0 is does not",row.names = TRUE)
```
...
If you want all the available files download the complete folder:
```{r, eval = TRUE}
# Get clinical data
query <- TCGAquery("ACC",platform = "bio", level = 2)
TCGAdownload(query)
# See downloaded files
print(dir(query$name,full.names = TRUE))
```
...
* **add.mutation.genes** Genes that are mutated into SummarizedExperiment objec? Default: `FALSE`
...
* **add.clinical** Add clinical information updated with last follow up information in the SummarizedExperiment object. See TCGAquery_clinic function for more information.
...
TCGAvisualize_oncoprint(mut = mut, genes = mut$Hugo_Symbol[1:30],
                        filename = "onco.pdf",
                        annotation = clin,
...
dataSmNT <- TCGAquery_SampleTypes(barcode = lsSample$IlluminaHiSeq_RNASeqV2,
                                  typesample ="NT")
# get clinical data
