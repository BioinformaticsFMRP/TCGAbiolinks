...
dataClin <- TCGAquery_clinic(cancer = cancer, clinical_data_type = "clinical_patient")
...
# Adding plot for mutation of a selected gene within Expression Clusters
...
TCGAvisualize_profilePlot(dataMut, 
                          ...
                          na.rm = FALSE,
                          filename = paste0(mRNAsel,"_Mutation_Variant_Classification_in EC clusters.pdf"))

TCGAvisualize_profilePlot(dataMut, 
                          groupCol = "groupsHC", 
                          subtypeCol =  "Variant_Type",
                          na.rm = FALSE,
                          filename = paste0(mRNAsel,"_Mutation_Variant_Type_in in EC clusters.pdf"))
...
#dataMut <- dataMut[dataMut$patient %in% names(consensusClusters),]

dataMut <- cbind(dataSubt, groupsHC = matrix(0,nrow(dataSubt),1))
rownames(dataMut) <- dataMut$patient
dataMut$patient <- as.character(dataMut$patient)

for( i in 1: nrow(dataMut)){
  currSmp <- as.character(dataMut$patient[i])
  dataMut[currSmp,"groupsHC"] <- as.character(consensusClusters[currSmp])
}

dataMut <- dataMut[dataMut$groupsHC!=0,]

dataMut$groupsHC <- as.character(dataMut$groupsHC)
...
dataMut$patient <- as.character(dataMut$patient)
dataMut$groupsHC <- as.factor(dataMut$groupsHC)
library(TCGAbiolinks)
TCGAvisualize_profilePlot(colData(coad.exp2), groupCol = "RNASeqCluster", subtypeCol =  "ATRX")

TCGAvisualize_mutation(data = colData(coad.exp2),groupCol = "RNASeqCluster",geneList = "IDH1",filename = "test.pdf",by = "cluster")
