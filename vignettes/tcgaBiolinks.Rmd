...
knitr::opts_chunk$set(comment = "", warning = FALSE, 
                      message = FALSE, echo = TRUE, tidy = FALSE, size="small")
...
The parameters that will be used to filter the data are:
...
```{r, eval = TRUE, echo = FALSE}
knitr::kable(head(tabPackage1), digits = 2, caption = "Table with number of downloads about a list of packages",row.names = FALSE)
...
* **mutation.genes** Add List of genes that are mutated for each patient? DEFAULT: `FALSE`
...
Firstly, it calculates the difference between the mean DNA methylation of each
group for each probes. 
...
`TCGAvisualize_Heatmap` will plot a heatmap and add to the samples bars representing
diferent groups.

The parameters of this function are:

* **data**	The object to with the heatmap data (expression, methylation)
* **metadata**	Dataframe with the labelCols and rownames with patients ids
* **consensusClusters**	One item of the returned from Concensus
* **labelCols**	Vector of columns to add to the heatpmap
* **cbPalette**	A list of colors, aech one will be used in the labelCols
* **clusterLabel**	The label of the cluster. Example "Expression Cluster",
* **filename**	Filename default "Heatmap.pdf"

For more information please take a look on case of study #2.
...
TCGAvisualize_SurvivalCoxNET can help an user to identify a group of survival
genes that are significant from univariate Kaplan Meier Analysis and also for 
Cox Regression.
It shows in the end a network build with community of genes with similar 
range of pvalues from Cox regression (same color) and that interaction among 
those genes is already validated in literatures using the STRING 
...
## `TCGAvisualize_profilePlot`: Analyzing expression and methylation together 

The best way to displaty the association between cancer subtypes 
and any kind of clustering is using the profilePlot function.

The inputs are:
* **data** Dataframe with at least two collumns (grupCol and subtypeCol)
* **groupCol** The name of the collumn with the group information
* **subtypeCol** The name of the collumn with the subtype information
* **  colors** A vector of colors 
* **na.rm** Remove the NA groups? DEAFULT: `FALSE`
...
Interpretation: if the cluster profiles (conditional distribution 
of the subtypes given the cluster)  are close to the subtype distribution 
(last bar) then there is no association between the subtypes and the clustering.
...
# Which sampels are primary solid Tumor
dataSmTP <- TCGAquery_SampleTypes(barcode = lsSample$IlluminaHiSeq_RNASeqV2,
                                  typesample = "TP")
# Which sampels are Solid Tissue Normal
...
Using  `TCGAnalyze_DEA`, we identified 3,390 differentially expression analysis (DEA) (log 
...
We focused on the analysis of LGG samples. In particular, we used TCGA biolinks
to download 293 samples with molecular subtypes. 
...
Firstly, we searched for possible outliers using the TCGAanalyze_Preprocessing 
function, which performs an Array Array Intensity correlation AAIC.
We used all samples in expression data with molecular subtypes,  filtering 
out sampels without molecular information, and using only IDHmut-codel (n=85),
...
According this matrix we found 0 samples with low correlation (cor.cut = 0.6) 
that can be identified as possible outliers, so we continued our analysis 
with 70 samples. 

Secondly using the `TCGAanalyze_Normalization` function we normalized mRNA 
transcripts and miRNA, using EDASeq package. This function does uses 
...
Thirdly using the `TCGAanalyze_Filtering` function we applied 3 filters removing 
features / mRNAs with low signal across samples obtaining 4578, 4284, 
1187 mRNAs respectively. 

Then we applied two Hierarchical cluster analysis on 1187 mRNAs after three 
filters described above, the first cluster using as method ward.D2, and the 
second with ConsensusClusterPlus.

After two clusters analysis, with cut.tree =4 we obtained n=4 espression clusters (EC).
...
symmetric matrix of pearson correlation among all samples (n=293). 
According this matrix we found 0 samples with low correlation (cor.cut = 0.6) 
that can be identified as possible outliers, so we continued our analysis with 70 samples. 

Secondly using the TCGAanalyze_Normalization function we normalized mRNA transcripts and miRNA, using EDASeq package. This function does uses Within-lane normalization procedures to adjust for GC-content effect (or other gene-level effects) on read counts: loess robust local regression, global-scaling, and full-quantile normalization (Risso et al., 2011) and between-lane normalization procedures to adjust for distributional differences between lanes (e.g., sequencing depth): global-scaling and full-quantile normalization (Bullard et al., 2010).

Thirdly using the TCGAanalyze_Filtering function we applied 3 filters removing features / mRNAs with low signal across samples obtaining 4578, 4284, 1187 mRNAs respectively. 
Then we applied two Hierarchical cluster analysis on 1187 mRNAs after three filters described above, the first cluster using as method ward.D2, and the second with ConsensusClusterPlus.
After two clusters analysis, with cut.tree =4 we obtained n=4 espression clusters (EC).
...
The next steps will be to visualize the data. Firstly, we created the survival plot.
```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
#----------- VISUALIZE --------------------
...
TCGAanalyze_survival(data = dataClin,
                     ...
                     legend = "RNA Group",
                     labels=levels(consensusClusters) ,
...
Secondly we'll create the profile plot for age, IDH.1p19q.Subtype and methylation cluster
for the EC groups.

```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
dataClin <- cbind(dataClin, AGE2 = matrix(0,nrow(dataClin),1))
dataClin[ dataClin[,"age_at_initial_pathologic_diagnosis"] <= 32, "AGE2"] <- "<=32 yr"
dataClin[ dataClin[,"age_at_initial_pathologic_diagnosis"] >= 41, "AGE2"] <- ">=41 yr"
dataClin[ dataClin$AGE2 == 0, "AGE2"] <- "33-40 yr"

clin_subt <- merge(dataClin,dataSubt, by.x="bcr_patient_barcode", by.y="patient",all.x = T)

TCGAvisualize_profilePlot(clin_subt,
                          groupCol="groupsHC",
                          subtypeCol="IDH.1p19q.Subtype",
                          filename = "case2_Idh_new.svg",
...
TCGAvisualize_profilePlot(clin_subt,
                          groupCol="groupsHC",
                          subtypeCol="MethylationCluster",
                          filename = "case2_Met.svg",
                          plot.margin = c(-4.0, -2.5, -0.5, 2),
                          na.rm = TRUE)
```
...
TCGAvisualize_profilePlot(clin_subt,
                          groupCol="groupsHC",
                          subtypeCol="AGE2",
                          filename = "case2_age_new.svg",
                          plot.margin = c(-4.0, -2.5, -0.5, 2),
                          na.rm = TRUE)
```

The age subgroup profile plot is shown below:
```{r, fig.width=6, fig.height=4, echo = FALSE, fig.align="center",hide=TRUE, message=FALSE,warning=FALSE}
library(png)
library(grid)
...
```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
TCGAvisualize_Heatmap(data = datFilt,
                      metadata = clin_subt,
                      consensusClusters= data_Hc2[[4]],
                      ...
                      cbPalette = list(c("cyan","green3","red","purple"),
                                     ...
                                     c("purple","green","red"),
                                     c("purple","green","red"),
                                     c("red","purple","green","orange","gray")))
```
...
dataMut <- cbind(LGGselected, groupsHC = matrix(0,nrow(LGGselected),1))
dataMut <- dataMut[!duplicated(dataMut$Tumor_Sample_Barcode),]
dataMut$Tumor_Sample_Barcode <- substr(dataMut$Tumor_Sample_Barcode ,1,12)
rownames(dataMut) <- dataMut$Tumor_Sample_Barcode

# Adding the Expression Cluster classification found before
for( i in 1: nrow(dataMut)){
    currSmp <- dataMut$Tumor_Sample_Barcode[i]
    dataMut[currSmp,"groupsHC"] <- as.character(consensusClusters[currSmp])
}

for( ccols in 1:length(  as.character(levels(consensusClusters)) )){
    dataMut[dataMut$groupsHC == as.character(levels(consensusClusters))[ccols],"groupsHC"] <- paste0("EC",ccols)
}

dataMut <- dataMut[dataMut$Variant_Classification!=0,]
```

You can see the for the groupsHC groups the distribution of the subtypes Variant_Classification of the gene
ATRX.
```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
TCGAvisualize_profilePlot(dataMut,
                          ...
                          na.rm = FALSE,
                          plot.margin = c(-4.0, -2.5, -0.5, 2),
                          filename = paste0(mRNAsel,"_Mutation_Variant_Classification_in_EC_clusters.png"))
```
...
library(png)
library(grid)
img <- readPNG("ATRX_Mutation_Variant_Classification_in_EC_clusters.png")
grid.raster(img)
```


You can see the for the groupsHC groups the distribution of the subtypes Variant_Type of the gene
ATRX.
...
You can see the for the RNASeqCluster groups the percentage of samples that have the ATRX mutated.
```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
...
You can see the for the RNASeqCluster groups the percentage of samples that have the IDH1 mutated.
```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
TCGAvisualize_mutation(data = colData(dataAssy),
                       groupCol = "RNASeqCluster",
                       geneList = "IDH1",
...
Firstly, we downloaded COAD methylation data for HumanMethylation27k and HumanMethylation450k
...
So, selected the groups CIMP.L and CIMP.H to do a expression and DNA methylation comparison.
...
For the methylation analysis, we do a DMR (different methylated region) analysis, which will give the 
...
With the object of assisting users in this arduous step, TCGAbiolinks will 
try to offer to prepare the data  in order to obtain the correct format for different packages.

An example of package to perform DNA methylation and expression analysis is ELMER [@ref1]. 
ELMER, which is designed  to combine DNA methylation and gene expression data from human tissues to infer 
multi-level cis-regulatory networks. ELMER uses DNA methylation to 
...
We will present this case study the study KIRC by TCGAbiolinks and 
...
kirc.met <- TCGAprepare_elmer(kirc.met,
                              ...
                              met.na.cut = 0.2)
```
For the expression  data we will search the platform IlluminaHiSeq_RNASeqV2
and level 3 data with TCGAquery. After, we will download the ormalized_results 
...
From this analysis it is possible to verify the relation between nearby 20 
...
Each scatter plot showing the average DNA methylation level of sites with the AP1 motif in all 
KIRC samples plotted against the expression of the transcription factor CEBPB and GFI1 respectively.
```{r,eval=FALSE,echo=TRUE,message=FALSE,warning=FALSE}
