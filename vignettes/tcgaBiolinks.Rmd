...
(expression/methylation analysis). The `TCGAprepare_elmer` for the DNA methylation data 
...
library(TCGAbiolinks)
path <- "kirc"query <- TCGAquery(tumor = "KIRC",level = 3, platform = "HumanMethylation450")
...
dataClin <- TCGAquery_clinic(cancer = cancer,
                             clinical_data_type = "clinical_patient")          # time = 1.060s

# selection of normal samples "NT"
samplesNT <- TCGAquery_SampleTypes(barcode = lsSample$IlluminaHiSeq_RNASeqV2,
                                   typesample = c("NT"))
# selection of tumor samples "TP"
samplesTP <- TCGAquery_SampleTypes(barcode = lsSample$IlluminaHiSeq_RNASeqV2,
                                   typesample = c("TP"))

TCGAdownload(data = datQuery, path = pathCancer, type = dataType,
             samples =c(samplesNT,samplesTP))

system.time(dataAssy <- TCGAprepare(query = datQuery,
                        dir = pathCancer,
                        type = dataType,
                        save = TRUE,
                        summarizedExperiment = TRUE,
                        samples = c(samplesNT,samplesTP),
                        filename = paste0(cancer,"_",PlatformCancer,".rda")))    # time = 1204.519s

save(dataAssy, file =paste0(pathCancer,("/dataAssyFull.Rdata")))

#load("../dataBrca/dataAssyFull.Rdata")

system.time(dataPrep <- TCGAanalyze_Preprocessing(object = dataAssy,cor.cut = 0.6))   # time = 48.423s

dataNorm <- TCGAanalyze_Normalization(tabDF = dataPrep,
                                      geneInfo = geneInfo,
                                      method = "gcContent")                  # time = 165.577s
save(dataNorm, file = "../dataBRCA/dataNormFull.Rdata")

load("../dataBrca/dataNormFull.Rdata")

dataFilt <- TCGAanalyze_Filtering(tabDF = dataNorm,
                                  method = "quantile",
                                  qnt.cut =  0.25)

datSmpNT <- intersect(samplesNT,colnames(dataFilt) )
datSmpTP <- intersect(samplesTP,colnames(dataFilt) )

system.time(dataDEGs <- TCGAanalyze_DEA(mat1 = dataFilt[,datSmpNT],
                            mat2 = dataFilt[,datSmpTP],
                            Cond1type = "Tumor",
                            Cond2type = "Normal",
                            fdr.cut = 0.01 ,
                            logFC.cut = 1,
                            method = "glmLRT"))                              # time = 223.936s

save(dataDEGs, file = "../dataBRCA/dataDEGsFull.Rdata")
load("../dataBrca/dataDEGsFull.Rdata")

# Enrichment Analysis EA
# Gene Ontology (GO) and Pathway enrichment by DEGs list
Genelist <- rownames(dataDEGs)
ansEA <- TCGAanalyze_EAcomplete(TFname="DEA genes Normal Vs Tumor",Genelist)    # time = 60.750s
...
dataSurv<-TCGAanalyze_SurvivalKM(clinical_patient = dataClin,
                                 ...
                                 ThreshTop = 0.67,
                                 ThreshDown = 0.33,
                                 p.cut = 0.05)       # time = 203.142s

save(dataSurv, file = "../dataBRCA/dataSurvFull.Rdata")
load("../dataBrca/dataSurvFull.Rdata")

load("/Users/antoniocolaprico/Downloads/org.Hs.string.RData")
system.time(TabCoxNet <- TCGAvisualize_SurvivalCoxNET(dataClin,dataFilt,
...
load("/Users/antoniocolaprico/Documents/gitHub/LGG_caseStudy/dataNorm.RData")
...
load("/Users/antoniocolaprico/Documents/gitHub/LGG_caseStudy/data_Hc2.Rdata")
...
#-------------------------------------------------
# 4.2 - DMR - Methylation analysis - volcano plot
# ------------------------------------------------
...
#------------------------------------------------
# 4.4 - DEA - Expression analysis - volcano plot
# -----------------------------------------------
...
TCGAVisualize_volcano(dataDEGs$logFC,dataDEGs$FDR,
                      ...
                      x.cut=5, names= rownames(dataDEGs),
