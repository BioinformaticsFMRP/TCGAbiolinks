...
## ----echo=TRUE, message=FALSE, warning=FALSE-----------------------------
query %>% getResults %>% dplyr::select(-matches("cases"))%>% datatable(options = list(scrollX = TRUE, keys = TRUE))
...
filter(clinical.index,submitter_id == "TCGA-F5-6702") %>% dplyr::select(vital_status,days_to_death,days_to_last_follow_up) %>% datatable
...
recurrent.samples <- GDCquery(project = "TCGA-LIHC",
                             ...
                             sample.type = 	"Recurrent Solid Tumor")$results[[1]] %>% dplyr::select(cases)
