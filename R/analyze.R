...
TCGAanalyze_DEA <- function (mat1, mat2, metadata = TRUE, Cond1type, Cond2type,
                            ...
                            FDRThresh = 0.01,GeneSymbolsTable= FALSE )
{
    ...
    table_pathway_enriched <- matrix(1, nrow(current_pathway_from_EA),
                                     8)
    colnames(table_pathway_enriched) <- c("Pathway", "GenesInPathway",
    ...
    for (i in 1:nrow(current_pathway_from_EA)) {
        ...
        genes_common_pathway_TFregulon <- as.matrix(intersect(toupper(RegulonList),
        ...
        if (length(genes_common_pathway_TFregulon) != 0) {
            ...
            if (FisherpvalueTF < 0.01) {
                ...
                current_pathway_commongenes_num_with_percent <- gsub(" ",
                                                                     "", paste(current_pathway_commongenes_num,
                ...
                table_pathway_enriched[i, "PercentPathway"] <- as.numeric(table_pathway_enriched[i,
                                                                                                 "CommonGenesPathway"])/as.numeric(table_pathway_enriched[i,
                                                                                                                                                          "GenesInPathway"]) * 100
                table_pathway_enriched[i, "PercentRegulon"] <- as.numeric(table_pathway_enriched[i,
                                                                                                 "CommonGenesPathway"])/length(RegulonList) *
                    100
            }
        }
    }
    ...
    if (nrow(table_pathway_enriched) > 0) {
        ...
        tmp <- paste(tmp[, "Pathway"], "; FDR= ", format(tmp[,
                                                             "FDR"], digits = 3), "; (ng=", round(tmp[, "GenesInPathway"]),
