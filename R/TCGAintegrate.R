...
TCGAintegrate<- function(querySamples){
    ...
    for ( i in 1: nrow(matSamples)){
        for( j in 1: nrow(matSamples)){
            if(i!=j){ matSamples[i,j] <- length(intersect(substr(querySamples[[i]],1,12), substr(querySamples[[j]],1,12)))}
        else{ matSamples[i,j] <- length(intersect(querySamples[[i]], querySamples[[j]]))}
        }
