...
#' @description Find most studied TF in pubmed related to a specific cancer, disease, or tissue
#' @param tumor tumor
  ...
  for (k in 1:nrow( dataDEGsFiltLevelTF)){
    ...
    if ( length(grep("No items found.",x))!=1){
      ...
      if (length(grep("Display Settings",x))==1){
        x6 <- 1
        dataDEGsFiltLevelTF[k,"PMID"] <- substr(gsub("</dt> <dd>","",unlist(strsplit(x,"PMID:"))[2]),1,8)
      ...
      if (length(grep("result_count",x))==1){
        ...
        dataDEGsFiltLevelTF[k,"PMID"]<- as.character(paste(substr(tmpPMID3,1,8),collapse="; "))
      ...
      print(paste("Cancer ", tumor, "with TF n. ",k, "of " ,nrow( dataDEGsFiltLevelTF)," : ", CurrentGene, "found n. ", x6, "pubmed."))

    }

    else{
      print(paste("Cancer ", tumor, "with TF n. ",k, "of " ,nrow( dataDEGsFiltLevelTF)," : ", CurrentGene, "no item found in pubmed."))
